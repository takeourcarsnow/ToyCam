...
function applyDithering(imageData, method, scale, palette) {
...
function floydSteinbergDithering(imageData, scale, palette) {
...
function bayerDithering(imageData, scale, palette) {
  const data = imageData.data;
  const width = imageData.width;
  const height = imageData.height;
  const step = Math.max(1, Math.floor(scale));
...
function atkinsonDithering(imageData, scale, palette) {
  const data = imageData.data;
  const width = imageData.width;
  const height = imageData.height;
  const step = Math.max(1, Math.floor(scale));
...
function orderedDithering(imageData, scale, palette, matrixSize = 8) {
  const data = imageData.data;
  const width = imageData.width;
  const height = imageData.height;
  const step = Math.max(1, Math.floor(scale));
...
function randomDithering(imageData, scale, palette) {
  const data = imageData.data;
  const width = imageData.width;
  const height = imageData.height;
  const step = Math.max(1, Math.floor(scale));
...
self.onmessage = function(e) {
  const { imageData, effect, settings } = e.data;
  
  if (effect === 'dithering') {
    const result = applyDithering(imageData, settings.method, settings.scale, settings.palette);
